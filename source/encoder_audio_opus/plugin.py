...
"""
    ...
    Written by:               Calvo  <francocalvo@tutanota.com>
    Based on work by:         Josh.5 <jsunnex@.com>
...
from encoder_audio_opus.lib.ffmpeg import StreamMapper, Probe, Parser

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.encoder_audio_opus")
...
class PluginStreamMapper(StreamMapper):
    ...
    def calculate_bitrate(stream_info: dict):
        ...
        # I adopted 254 per https://wiki.xiph.org/Opus_Recommended_Settings 
        # for Music Storage 5.1.
        if not channels:
            logger.debug("Stream did not contain 'channels'. Setting 254K bitrate (6).")
            return (254*6)

        return (int(stream_info.get('channels')) * 254)
    ...
    def custom_stream_mapping(self, stream_info: dict, stream_id: int):
        ...
        if settings.get_setting('advanced'):
            ...
            if stream_info.get('channels'):
                # Use 254K for the bitrate per channel
